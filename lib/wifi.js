...
module.exports = {
    ...
    info(getQrcode = false){ // 如果 getQrcode === true，就返回一个Promise。否则，就是一个同步方法。
        const data = {
            ip: this.getLocalIp(),
        ...
        if (getQrcode) {
            return co(function *(){
                const dataURL = yield toDataURL(JSON.stringify(_.omit(data, 'connectionCount')));
                return _.extendOwn(data, {dataURL});
            });
        }
        return data;
    },
