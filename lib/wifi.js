...
module.exports = {
  ...
  start({tempPath, port = 8686, onConnection, onClose}){
    ...
    server.listen(port, () => {
      logInfo(`敏行服务: ${JSON.stringify(this.getLocalIp())} port: ${server.address().port}`);
    });
  },
  end(){
  ...
  afterSocketConnect({socket, onConnection, onClose}){
    ...
    logInfo(`有设备连接, 当前连接设备数:${moduleOptions.connectionCount}, port:${moduleOptions.port}`);

    socket.send(JSON.stringify(this.httpPortCmd({port: moduleOptions.port})));

    socket.on('error', err => logErr(err));
