...
module.exports = {
  ...
  info(){
    return _.defaults({
      ip: this.getLocalIp()
    }, _.pick(moduleOptions, 'port', 'connectionCount', 'remoteIps'));
  },
  ...
  afterSocketConnect({socket, onConnection, onClose}){
    ...
    moduleOptions.remoteIps.push(socket._sender._socket.remoteAddress);
    ...
    socket.on('error', logErr).on('close', function(){
      const delIndex = moduleOptions.remoteIps.indexOf(this._sender._socket.remoteAddress);
