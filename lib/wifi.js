...
const buildHtmlPage = co.wrap(function *(title, ips, dataURLs){
    let source = yield fsp.readFile(path.join(__dirname, 'qrcodes.html'));
    if (Buffer.isBuffer(source)) {
        source = source.toString('utf-8');
    }
    const template = handlebars.compile(source);
    return template({title, ips, dataURLs});
});
...
module.exports = {
    ...
    info(qrCodeTitle){ // 如果 qrCodeTitle 是 非空字符串，就返回一个Promise。否则，就是一个同步方法。
        const data = {
            ...
            remoteIps: moduleOptions.remoteIps
        };
        if (_.isString(qrCodeTitle) && !_.isEmpty(qrCodeTitle)) {
            return co(function *(){
                yield fsp.mkdirs(QRCODE_TEMP);
                const {ip: ips, port} = data;
                const ipsStr = ips.join('_');
                const qrcodeFileName = `${ipsStr}_${data.port}.html`;
                ...
                if (!exists) {
                    const dataURLs = yield Promise.all(ips.map(ip => toDataURL(`native://mxDevApp?${qs.stringify({
                        ip, port, type: 'connect-minxing-devtool'
                    })}`, {scale: 8})));
                    const html = yield buildHtmlPage(qrCodeTitle, ips, dataURLs);
    ...
    afterSocketConnect({socket, onConnection, onClose}){
        ...
        _.isFunction(onConnection) && onConnection(moduleOptions.remoteIp);
        ...
        socket.on('error', logErr).on('close', function(){
            ...
            _.isFunction(onClose) && onClose(moduleOptions.remoteIp);
        }).on('message', message => {
            let incoming = '';
            try {
    ...
    sync({project, updateAll}){ // 更新,全量或增量.
        ...
        if (Fse.existsSync(configPath)) { // eslint-disable-line no-sync
            const appInfo = Utils.readPropertiesSync(configPath); // eslint-disable-line no-sync
            if (appInfo) {
    ...
    preview({file}){ // 页面实时预览.
        ...
        const {app_id: appId, project} = Utils.fetchProjectRootInfoByFile(file);
        if (!appId) {
