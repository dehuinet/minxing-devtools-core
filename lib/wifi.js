...
module.exports = {
  ...
  start({tempPath, port = 8686, onConnection, onClose}){
    ...
    const server = require('http').createServer((req, res) => {
      ...
      logDebug('file stat size->', stat.size);
    ...
    }).listen(port, () => logInfo(`敏行服务: ${JSON.stringify(this.getLocalIp())} port: ${server.address().port}`));
    const wss = new WebSocketServer({server}).on('connection', socket => this.afterSocketConnect({
      socket, onConnection, onClose,
      remoteIps: wss.clients.map(client => client._socket.remoteAddress)
    }));
  ...
  afterSocketConnect({socket, onConnection, onClose, remoteIps}){
    ...
    _.extendOwn(moduleOptions, {
      connectionCount: moduleOptions.connectionCount + 1,
      remoteIps
    });
    _.isFunction(onConnection) && onConnection();

    logInfo(`有设备连接, 当前连接设备数:${moduleOptions.connectionCount}, 连接主机: ${JSON.stringify(remoteIps)}, port:${moduleOptions.port}`);

    socket.send(JSON.stringify(this.httpPortCmd({port: moduleOptions.port})));

    socket.on('error', logErr).on('close', () => {
      moduleOptions.connectionCount -= 1;
      logInfo(`有设备断开,当前连接设备数:${moduleOptions.connectionCount}`);
