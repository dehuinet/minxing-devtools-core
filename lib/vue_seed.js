...
const VUE_DEFAULT_TEMPLATE_PATH = '../vue_template';
...
function add({
    ...
    } else if (tu.isLocalFileOrigin(origin)) {
        from = tu.getFilePath(origin);
    }
    if (!fse.existsSync(from)) {
        return Promise.resolve({err: `没有找到来自${origin}的模版文件！`});
    } else {
        return copy({
            from,
            name,
