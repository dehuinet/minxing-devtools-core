...
const Package = {
    ...
    getSmartCmd({
        ...
        return this.getProjectModifyTimestampAndFiles({
            ...
            const filePath = path.join(tempPath, projectPath, fileName);
            ...
            if (!sToE) {
                ...
                return Promise.resolve({
                    command: 3,
                    zipPath: path.join(tempPath, projectPath, `${sToE}.zip`),
                    appId: appId,
    ...
    zipSmartPackage({
        ...
        return new Promise(resolve => {
            ...
            if (fileList.length === 0) {
                ...
                const tempAppPath = path.join(tempPath, projectPath, name);
                const copiedFiles = [];
                    ...
                    fse.copy(filePath, targetPath, (e) => {
                        ...
                        if (copiedFiles.length === fileListCount) {
                            MXBuild.build({
                                ...
                                savePath: path.join(tempPath, projectPath),
                                projectName: name
                            }).then(() => {
                                this.$zipPhase = false;
                                resolve(startToEnd)
    ...
    getSyncCmd({
        ...
        .then(newTimestamp => {
            return Promise.resolve({
                command: 3,
                zipPath: path.join(tempPath, projectPath, `${newTimestamp}.zip`),
                appId: appId,
