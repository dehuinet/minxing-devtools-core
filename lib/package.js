    ...
    getProjectModifyTimestampAndFiles({
        ...
        return new Promise((resolve, reject) => {
            ...
            const time = fileList.reduce((timestamp, file) => {
                ...
                ctime = Math.floor(ctime.getTime()/ 1000);
                // 取创建时间和修改时间中，最新的时间
    ...
    zipSmartPackage({
        ...
        return new Promise(resolve => {
            ...
            fileList = fileList.filter(file => {
                ...
                const validateTime = mtime >= ctime ? mtime : ctime;
                return Math.floor(validateTime.getTime()/ 1000) > start;
            });
                ...
                fileList.forEach(filePath => {
                    ...
                    fse.copy(filePath, targetPath, (e) => {
                        ...
                        if (copiedFiles.length === fileListCount) {
                            const target = path.resolve(projectOfTempPath, `${name}.zip`)
                            MXBuild.zipPackage({
                                target,
                                origin: tempAppPath
                            }).then(() => {
                                ...
                                console.log('zip package error->', e);
                                this.$zipPhase = false;
                            })
                        }
                    })
